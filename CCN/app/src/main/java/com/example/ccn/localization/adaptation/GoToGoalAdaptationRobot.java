...
public class GoToGoalAdaptationRobot implements Robot {
    @NonNull private static final String TAG = "GoToGoalAdaptationRobot";
    @NonNull private final GoToGoalAdaptationMachine machine;
    @NonNull private final GoToGoalAdaptationScreen screen;
    ...
    GoToGoalAdaptationRobot(@NonNull GoToGoalAdaptationMachine machine, GoToGoalAdaptationScreen screen) {
    ...
    public Future<Void> stop() {
        machine.post(GoToGoalAdaptationEvent.STOP);
    ...
    public void start(@NonNull QiContext qiContext) {
        ...
        this.screen.activity.adaptation = true;

        this.screen.activity.first_end_phase = false;

        machine.post(GoToGoalAdaptationEvent.START);
    ...
    private void goToGoalAdaptationFrame() {
        if (qiContext == null) {
            Log.e(TAG, "qiContext is null");
            machine.post(GoToGoalAdaptationEvent.GO_TO_GOAL_ADAPTATION_FAILED);
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                ClientManager.reset(qiContext, success -> {
                    ...
                    ClientManager.postParadigm(qiContext, culture, "adaptation", successParadigm -> {
                        if (successParadigm) {
                            Log.d(TAG, "Paradigm posted successfully!");

                        } else {
                            Log.e(TAG, "Failed to post paradigm.");
                        }
                        qiContext.getMapping().async().mapFrame()
                                .thenConsume(mapFrameFuture -> {
                                    ...
                                    try {
                                        ...
                                        screen.activity.releaseAutonomousAbilities();
    ...
    public void moveToNextNode() {
        JSONObject response = ClientManager.getProxemicChanged(qiContext);
        boolean changed = response.optBoolean("changed", false);
        if (changed) {
            JSONArray smooth = response.optJSONArray("smooth_path");
            pathToGoal = smooth;  // ✅ this will now work safely
        }
        ...
        if (index >= pathToGoal.length()) {
            ...
            if (!returning) {
                ...
                if (speak_){
                    ...
                    phase = 2;
                    this.screen.activity.phase = phase;
                    //conversation(2); //still in navigation
                }
                screen.activity.releaseAutonomousAbilities();
                returning = !returning;
                moveBackToStart();
                ...
                screen.activity.runOnUiThread(() -> {
                    screen.activity.holdAutonomousAbilities();
                });
;
                machine.post(GoToGoalAdaptationEvent.GO_TO_GOAL_ADAPTATION_SUCCEEDED);
            }
            return;
        }
        try {
            JSONObject current = pathToGoal.getJSONObject(index);
            ...
            GoToBuilder.with(qiContext)
                    ...
                    .thenConsume(future -> {
                        ...
                        float oneThird = ((float )pathToGoal.length()) / ((float)n_conv_phases);
                        if (phase == 0 || phase == 1) {
                            phase++;
                        }
                        this.screen.activity.phase = phase;
                        ...
                        if (index + index_movement >= pathToGoal.length()){
                            if (index <= pathToGoal.length() - 2){
                                index++;
                                moveToNextNode();
                            }
                            else{
                                index = pathToGoal.length();
                                moveToNextNode();
                            }
                        }
                        else{
                            index = index + index_movement;
                            moveToNextNode();
                        }
                    });
    ...
    private void onGoToGoalAdaptationStateChanged(@NonNull GoToGoalAdaptationState goToGoalState) {
        switch (goToGoalState) {
            ...
            case MOVING:
                cancelCurrentActions()
                        .andThenConsume(ignored -> goToGoalAdaptationFrame());
                break;
            case ERROR:
                cancelCurrentActions();
            ...
            case SUCCESS:
                cancelCurrentActions()
                        .andThenConsume(ignored -> machine.post(GoToGoalAdaptationEvent.SUCCESS_CONFIRMED));
                break;
        }
    }
}
