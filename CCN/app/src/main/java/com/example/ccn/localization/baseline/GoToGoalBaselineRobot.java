...
import com.example.ccn.localization.baseline.GoToGoalBaselineEvent;
...
public class GoToGoalBaselineRobot implements Robot {
    @NonNull private static final String TAG = "GoToGoalBaselineRobot";
    @NonNull private final GoToGoalBaselineMachine machine;
    @NonNull private final GoToGoalBaselineScreen screen;
    ...
    GoToGoalBaselineRobot(@NonNull GoToGoalBaselineMachine machine, GoToGoalBaselineScreen screen) {
    ...
    public Future<Void> stop() {
        machine.post(GoToGoalBaselineEvent.STOP);
    ...
    public void start(@NonNull QiContext qiContext) {
        ...
        machine.post(GoToGoalBaselineEvent.START);
    ...
    private void goToGoalBaselineFrame() {

        if (qiContext == null) {
            Log.e(TAG, "qiContext is null");
            machine.post(GoToGoalBaselineEvent.GO_TO_GOAL_BASELINE_FAILED);
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                ClientManager.reset(qiContext, success -> {
                    ...
                    ClientManager.postParadigm(qiContext, "English", "baseline", successParadigm -> {
                        if (successParadigm) {
    ...
    private void moveToNextNode(int index, JSONArray nodes) {
        ...
        if (index >= nodes.length()) {
            ...
            if (!returning) {
                ...
                machine.post(GoToGoalBaselineEvent.GO_TO_GOAL_BASELINE_SUCCEEDED);
            }
            return;
        }
        try {
    ...
    private void onGoToGoalBaselineStateChanged(@NonNull GoToGoalBaselineState goToGoalBaselineState) {
        switch (goToGoalBaselineState) {
            ...
            case MOVING:
                cancelCurrentActions()
                        .andThenConsume(ignored -> goToGoalBaselineFrame());
                break;
            case ERROR:
                cancelCurrentActions();
            ...
            case SUCCESS:
                cancelCurrentActions()
                        .andThenConsume(ignored -> machine.post(GoToGoalBaselineEvent.SUCCESS_CONFIRMED));
                break;
        }
    }
}
