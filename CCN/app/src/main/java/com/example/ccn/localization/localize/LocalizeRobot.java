 ...
 */
public class LocalizeRobot implements Robot {
    ...
    private Future<Void> say(@StringRes int resId) {
        return FutureCancellations.cancel(speech)
                .andThenCompose(ignored -> {
                    if (qiContext == null) {
                        throw new IllegalStateException("qi Context is null");
                    }
        ...
        switch (localizeState) {
            ...
            case ADVICES:
                //say(R.string.localize_advices_speech)
                        //.andThenConsume(ignored -> say(R.string.countdown_speech))
                        //.andThenConsume(ignored -> machine.post(LocalizeEvent.ADVICES_ENDED));
                machine.post(LocalizeEvent.ADVICES_ENDED);
                break;
            case LOADING_MAP:
                if (qiContext == null){
            ...
            case LOCALIZING:
                ...
                localizeManager.localizeRobot(qiContext)
                        .thenConsume(future -> {
                            if (future.isSuccess()) {
            ...
            case SUCCESS:
                ...
                machine.post(LocalizeEvent.SUCCESS_CONFIRMED);
                break;
        }
    }
}
