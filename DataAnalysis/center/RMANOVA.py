...
if __name__ == "__main__":
    ...
    def perform_repeated_measures_tests(df, column, subject_col="Participant ID", within_col="P"):
        ...
        try:
            ...
            # try to extract main F and p-value from the first row
            try:
                row0 = aov.iloc[0]
                F = row0.get('F', np.nan)
        ...
        try:
            ...
            # iterate rows and append results
            ...
            # print a concise table if possible
    ...
    # Also run Repeated Measures on the within-subject factor P
    results_all = []
    ...
    try:
        ...
        out_path = os.path.join(out_dir, 'RMANOVA.csv')
        if results_all:
            df_results = pd.DataFrame(results_all)
            df_results.to_csv(out_path, index=False)
            print(f"\nSaved RMANOVA results to: {out_path}")
        else:
