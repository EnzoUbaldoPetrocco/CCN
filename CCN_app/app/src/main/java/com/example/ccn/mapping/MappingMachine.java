...
class MappingMachine {
    ...
    @NonNull
    private static final String TAG = "MappingMachine";
    ...
    void post(@NonNull MappingEvent event) {
        ...
        Log.d(TAG, "New state in post is: " + newState.toString());
    ...
    private MappingState reduce(@NonNull MappingState currentState, @NonNull MappingEvent event) {
        Log.d(TAG, "Mapping event is :" + event.toString());
        switch (event) {
            case FOCUS_GAINED:
            ...
            case START_MAPPING:
                Log.d(TAG, "Mapping event is START_MAPPING");
